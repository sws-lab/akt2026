...
public class ShologCompilerTest {
    ...
    public void test07_variables_error() {
        checkCompileError(var("jama"), -UNDEFINED_CODE);
        checkCompileError(eand(var("b"), var("jama")), 0, -UNDEFINED_CODE);
        checkCompileError(eor(var("a"), var("jama")), 1, -UNDEFINED_CODE);
        checkCompileError(xor(var("a"), var("jama")), 1, -UNDEFINED_CODE);
        checkCompileError(xor(var("b"), var("jama")), 0, -UNDEFINED_CODE);
    }

    @Test
    public void test08_error_multiple() {
        checkCompileError(eand(error(1), error(2)), -1);
        checkCompileError(eor(error(1), error(2)), -1);
        checkCompileError(xor(error(1), error(2)), -1);
        checkCompileError(land(error(1), error(2)), -1);
        checkCompileError(lor(error(1), error(2)), -1);

        checkCompile(xor(land(var("b"), error(1)), lor(var("a"), error(2))), true);
        checkCompileError(xor(land(var("a"), error(1)), lor(var("a"), error(2))), 1, -1);
        checkCompileError(xor(land(var("b"), error(1)), lor(var("b"), error(2))), 0, 0, -2);
        checkCompileError(xor(land(var("a"), error(1)), lor(var("b"), error(2))), 1, -1);
        checkCompileError(xor(land(var("a"), error(UNDEFINED_CODE)), lor(var("b"), error(2))), 1, -UNDEFINED_CODE);
        checkCompile(xor(land(var("b"), var("jama")), lor(var("a"), var("jura"))), true);

        checkCompileError(xor(eand(var("b"), error(1)), lor(var("a"), error(2))), 0, -1);
        checkCompileError(xor(land(var("b"), error(1)), eor(var("a"), error(2))), 0, 1, -2);
    }
