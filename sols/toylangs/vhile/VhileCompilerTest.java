...
public class VhileCompilerTest {
    ...
    public void test07_multiple_no_escape() {
        checkCompile(assign("x", num(2048)), Map.of("x", 2048));
        checkCompile(assign("x", var("z")), Map.of("x", 3));

        checkCompile(assign("x", add(num(2), mul(num(2), num(3)))), Map.of("x", 8));
        checkCompile(assign("x", mul(add(num(2), num(3)), num(3))), Map.of("x", 15));

        checkCompile(loop(var("x"), block(assign("y", num(10)), assign("x", num(0)))), Map.of("x", 0, "y", 10));
        checkCompile(loop(var("z"), block(assign("y", num(10)), assign("z", num(0)))), Map.of("z", 0, "y", 10));
    }

    @Test
    public void test08_multiple_escape() {
        checkCompile(loop(neq(var("x"), num(10)), block(assign("x", add(var("x"), num(1))), escape(1), assign("x", add(var("x"), num(1))))), Map.of("x", 2));
        checkCompile(block(loop(num(1), escape(2)), assign("x", num(5))), Map.of());
        checkCompile(block(loop(num(1), block(assign("y", num(7)), escape(2))), assign("x", num(5))), Map.of("y", 7));
